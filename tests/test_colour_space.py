...
def test_q1e_can_resample_chroma():
    ...
    assert cbcr.shape[0] == y.shape[0] # assert cbcr.shape[0] == y.shape[0] // 2
    assert cbcr.shape[1] == y.shape[0] # assert cbcr.shape[1] == y.shape[0] // 2
...
def test_q1f_invalid_ycbcr_format_throws_error():
    ...
    with pytest.raises(ValueError):
        Y = np.zeros((3, 3))
        cbcr = np.zeros((3, 3, 3)) # cbcr = np.zeros((3, 3, 3))
        converter.to_rgb(Y, cbcr)
